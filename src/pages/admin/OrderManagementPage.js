...
function OrderManagementPage() {
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.get('/api/admin/orders');  // API 호출
    ...
    const cancelOrder = async (orderId) => {
        try {
            await axios.delete(`/api/orders/${orderId}`);  // 취소 API 호출
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        try {
            await axios.put(`/api/admin/orders/${orderId}/status`, { newStatus });  // 상태 변경 API 호출
